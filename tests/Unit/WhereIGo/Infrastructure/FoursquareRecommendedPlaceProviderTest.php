...
class FoursquareRecommendedPlaceProviderTest extends TestCase
{
    public function testGetRecommendedPlacesByCity()
    {
        // Create a mock
        ...
        $fourSquare = new FoursquareRecommendedPlaceProvider($client);
        $places = $fourSquare->getRecommendedPlacesByCity("Calgary, CA");

        $this->assertEquals("Russ & Daughters", $places->first()->getName());
        $this->assertEquals("Gourmet Shop", $places->first()->getCategories()[0]->name);
        $this->assertEquals(1, count($places->first()->getCategories()));
        $this->assertEquals("179 E Houston St (btwn Allen & Orchard St) | New York, NY 10002 | United States", $places->first()->getAddress());
    }
